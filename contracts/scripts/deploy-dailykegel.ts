...
async function main() {
  ...
  if (!tokenAddress) {
    ...
    console.error("  COOLDOWN=300 UU_TOKEN=0x... npx hardhat run scripts/deploy-dailykegel.ts --network bscTestnet");
    console.error("  START_TIME=1234567890 COOLDOWN=300 UU_TOKEN=0x... npx hardhat run scripts/deploy-dailykegel.ts --network bscTestnet");
    process.exit(1);
  }
